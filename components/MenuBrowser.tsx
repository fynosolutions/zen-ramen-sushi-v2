...
export default function MenuBrowser(){
  ...
  {menus.map(m=><div key={m.id} id={'panel-'+m.id} role="tabpanel" aria-labelledby={'tab-'+m.id} hidden={current!==m.id} tabIndex={0}>
    {m.id==='happy-hour'&&<p className="hh-banner"><strong>EVERYTHING $6.49</strong> CASH · $6.75 CARD — <span style={{whiteSpace:'nowrap'}}>DAILY 4–8 PM</span> <a href="/happy-hour/">SEE THE DEAL ↗</a></p>}<div className="menu-meta"><p>{m.availability}</p><a href={m.pdf} target="_blank" rel="noopener noreferrer">VIEW ORIGINAL PDF ↗</a></div>
